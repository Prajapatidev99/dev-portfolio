...
const AdminModal: React.FC<AdminModalProps> = ({ isOpen, onClose, onAdd }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-6">
      ...
      <div className="relative w-full max-w-2xl bg-zinc-900 border border-white/10 rounded-[2.5rem] p-8 md:p-12 shadow-2xl max-h-[90vh] overflow-y-auto custom-scrollbar">
        <div className="flex justify-between items-center mb-10">
          <div>
            <h2 className="text-3xl font-bold tracking-tight">Expand Portfolio</h2>
            <p className="text-zinc-500 mt-1">Add a high-impact project to your showcase.</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="md:col-span-2">
              ...
              <input 
                required
                className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 focus:outline-none focus:border-white/30 transition-all font-medium text-lg"
          ...
          <div>
            ...
            <textarea 
              className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 focus:outline-none focus:border-white/30 transition-all h-40 resize-none text-zinc-300"
          ...
          <button 
            type="submit"
            className="w-full py-6 bg-white text-black rounded-2xl font-black text-lg hover:bg-zinc-200 transition-all flex items-center justify-center gap-3 active:scale-[0.98]"
          >
            <Plus size={24} />
